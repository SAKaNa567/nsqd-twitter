...
func readFromTwitter(votes chan<- string)  {
	options, err := loadOptions()//全ての投票での選択肢を取得
	if err != nil {
	...
	req, err := http.NewRequest("POST",u.String(),//*http.Requestを作成(url.ValuesオブジェクトがエンコードされたものをPOSTリクエストとして送信する必要がある。
...
func startTwitterStream (stopchan <-chan struct{}, votes chan<- string) <-chan struct{} {
		stoppedchan := make(chan struct{}, 1)//シグナルのチャネル
		go func() {
			defer func(){
				stoppedchan <- struct{}{}
			}()
			for {
				select {
					case <-stopchan:///stopchanへの受信がある場合。
